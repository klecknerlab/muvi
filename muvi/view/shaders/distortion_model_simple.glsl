...
vec3 distortion_map(in vec3 U) {
    float exy = 0.25 * (distortion_correction_factor.x * (1.0 - 2.0 * U.x) + distortion_correction_factor.y * (1.0 - 2.0 * U.y));
    float ez = 0.25 * (distortion_correction_factor.z * (1.0 - 2.0 * U.z));
