# Reader/writer for custom "sparse" data files, used by the original v4d code.
# This is now depreciated, but included for backwards compatibility.
...
def make_block(a, sparse=None):
    ...
    header = b''.join([
        bytes('%-7s%c' % ('SPARSE' if sparse else 'ARRAY', a.dtype.char), encoding='utf-8'),
...
"""
    ...
    def __init__(self, fn, read_write='r', header_dict=None, max_blocks=1024, cache_blocks=False, preload=False):
        ...
        if read_write == 'w':
            ...
            self.f.write(bytes(self.file_id + '\n' + '#%s\n#Brief description below.\n\n' % self.file_desc, encoding='utf-8'))
            for k in sorted(self.header.keys()):
                self.f.write(bytes(k + ': ' + self.header[k] + '\n', encoding='utf-8'))
            self.f.write(bytes(self.post_comments + "\n>>>BEGIN BINARY>>>\n", encoding='utf-8'))
            self.zero_offset = self.f.tell()
            self.write_struct('Q', max_blocks)
            self.f.write(b'\x00' * 8 * (max_blocks + 1))
...
#                key, value = map(str.strip, line.split(':', 1))
    ...
    def unpack_block(self, desc, extra_header, data):
        if desc.startswith(b'ARRAY') or desc.startswith(b'SPARSE'):
            dtype = str(desc[-1:], encoding='utf-8')
            ndims = struct.unpack('Q', extra_header[:8])[0]
            shape = struct.unpack('%dQ' % ndims, extra_header[8:])

            if desc.startswith(b'SPARSE'):
                return sparse_decode(data, shape, dtype)
            else:
                return fromstring(data, dtype=dtype).reshape(shape)
        else:
    ...
    def read_header_line(self):
        ...
        while len(line) == 0 or line[0] == '#':
            ...
            line = str(b''.join(l), encoding='utf-8')
...
class Sparse4D(TextHeaderedBinary):
    ...
    def frame(self, n):
        arr = self[n]
        # print('FRAME: %d'%n, repr(arr.shape))
        return self[n]
...
pack_re = re.compile(b'\x00{1,65534}', flags=re.S) #Should be 65535 = 2^16-1, but this causes a regex overflow!
#pack_re = re.compile('\x00{1,255}', flags=re.S) #Debug
unpack_re = re.compile(b'(\x00\x00..|\x00.)', flags=re.S)
# lots_o_zeros = chr(0) * 65536
lots_o_zeros = bytes(65536)
...
#    parser.add_argument('-o', dest='output', type=str, default="%s.s4d", help='output filename')
    ...
    if list(args.input)[0] == 'TEST':
        for ft in (TextHeaderedBinary, Sparse4D):
            test = ft('test.thb', 'w', {'test':(1, 2, 3), 'another test':'abc'})
            test.append_array(clip(arange(100)%5, 3, 4)-3)
